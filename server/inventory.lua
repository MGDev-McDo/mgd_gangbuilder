if Config.ox_inventory then
    ...
    ox_inventory:registerHook('swapItems', function(payload)
        ...
        local formatedName = 'gang_'.. xPlayer.gang.name

        if payload.fromInventory == formatedName then
            return xPlayer.gang.grade_permissions['perm_inventory_take']
        end

        if payload.toInventory == formatedName then
            return xPlayer.gang.grade_permissions['perm_inventory_put']
        end
    end)
else
        ...
        TriggerEvent('esx_datastore:getSharedDataStore', 'gang_'.. xPlayer.gang.name, function(store)
            ...
            if putType == "items" then
                if xPlayer.getInventoryItem(putName).count >= putQuantity then
                    ...
                    cb(false, _('server_error_putItem_items', putLabel), 'error')
                    return
                end
            end
            ...
            if putType == "weapons" then
                ...
                successText = _('server_success_putItem_weapons', putLabel, putQuantity)
                removeCheck = true
                TriggerEvent('mgd_gangbuilder:dLogs', "GANG", {
                    author = _src,
                    action = "gangPutInventory",
            ...
            if putType == "accounts" then
                if xPlayer.getAccount(putName).money >= putQuantity then
                    ...
                    cb(false, _('server_error_putItem_accounts', putLabel), 'error')
                    return
                end
            end
            ...
            if foundInInv then
                if takeType == "items" then
                    if inv[foundIndex].count >= takeQuantity then
                        if xPlayer.canCarryItem(takeName, takeQuantity) then
                            ...
                            cb(false, _('server_error_takeItem_items'), 'error')
                            return
                        end
                    else
                        cb(false, _('server_error_takeItem_quantity', ESX.GetItemLabel(takeName)), 'error')
                        return
                    end
                end
                ...
                if takeType == "accounts" then
                    if inv[foundIndex].count >= takeQuantity then
                        ...
                        cb(false, _('server_error_takeItem_quantity'), 'error')
                        return
                    end
                end
                ...
                cb(false, _('server_error_takeItem'), 'error')
            end
        end)
    end)
end